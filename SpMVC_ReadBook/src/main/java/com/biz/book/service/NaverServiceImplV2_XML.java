...
public class NaverServiceImplV2_XML extends NaverServiceImplV1 {
	...
	// 여기에 아무런 코드가 없어도 프로젝트가 정상 수행이 되고있다.

	// 3의 method중에서 getNaverList(jsonString) method만 upgrade하겠다.
	@Override
	public List<BookVO> getNaverList(String queryURL) {

		// queryURL(요청주소포함된)문자열을 URI객체로 만들어
		// http 프로토콜에서 사용 할 수 있도록 만드는 클래스 
		// 기존에 URL클래스가 있으나 새로운 기능을 수행하기 위해서 
		// 별도로 URI클래스를 만들어놨으며 많은 기능들이 추가되어있다.
		// restTemplate를 사용하기 위해서는 
		// queryURL 문자열을 URL객체가 아닌 URI객체로 사용해야 한다. 
		URI restURI = null;
		 ...
		 * framework프로젝트에서 사용할 수 있도록 만들어 두었다. 
		 * 
		 * HttpHeader, HttpEntity, ResponseEntity객체만 잘 작성하면
