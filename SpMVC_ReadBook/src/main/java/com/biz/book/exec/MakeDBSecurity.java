...
/*
 * jasypt 패키지의 StandardPBEStringEncryptor클래스를 사용하여
...
public class MakeDBSecurity {
	...
	public static void main(String[] args) {
		...
		if(saltPassword == null) {
			System.out.println("BIZ.NET환경변수 설정값이 없습니다. 확인하세요");
			return;
		}
		System.out.println("BIZ.NET 환경변수  : %s\n" + saltPassword);
		 ...
		 * 
		 * salt값을 일반 문자열로 사용하면
		 * 소스코드에 문자열이 노출되어 각종 값들을 
		 * 암호화 하는 용도로 사용되는데 문제가
		 * 발생 할 수 있다.
		 * 그래서 일반적으로는 문자열을 바로 사용하지 않고
		 * 시스템(컴퓨터운영체제)에 환경변수를 저장해주고 
		 * 환경변수를 가져다 salt비밀번호로 사용을 한다. 
		 * 
		 *  BIZ.COM이라는 문자열이 노출이 되어도 
		 *  네트워크를 통해서 salt암호를 추출하기가 매우 어려워진다. 
		 *  
		 *  따라서 암호화 하는 효과를 좀더 배가 시킬 수 있다. 
		 */
